...
class Usuario:
    ...
    def Cadastrar(self):
        ...
        # Cadastro do nome
        self.nome = self.CadastroNome()
        # Cadastro do email
        self.email = self.CadastroUsuario()
        # Cadastro da senha
        self.senha = self.CadastroSenha()

        # Escrever todos os cadastros no bancodedados.txt
        with open('bancodedados.txt', 'a', encoding = 'utf-8') as arquivo:
            arquivo.write(f'{self.nome},{self.email},{self.email}\n')
            ...
            for linha in arquivo:
                ...
                if len(partes) >= 3:
                    ...
                    if email == email_input:
                        while True:
                            senha_log = self.InputSenha('Senha: ').strip()
                            if senha_log == senha:
                                self.nome = nome
                                self.email = email
                                self.senha = senha
                                Menu.LimparTerminal()
                                menu_global.MenuPrincipal(self.email)
                                return
                            else:
                                Menu.LimparTerminal()
                                console.print(Panel('[bold red]❌ Senha inválida.[/bold red]', border_style='red', width=60))
